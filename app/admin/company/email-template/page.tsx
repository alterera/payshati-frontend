...
export default function EmailTemplatePage() {
  ...
  const loadTemplates = async () => {
    try {
      setIsLoading(true);
      const response = await adminApi.listEmailTemplates(loginKey!, userId!);
  ...
  const handleEdit = async (id: number) => {
    const response = await adminApi.getEmailTemplate(loginKey!, userId!, id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await adminApi.updateEmailTemplate(loginKey!, userId!, {
        ...formData,
