...
import { reportsApi, AccountReportItem } from '../../../../lib/api/reports';
...
export default function AccountReportsPage() {
  const [reports, setReports] = useState<AccountReportItem[]>([]);
              ...
              {reports.length === 0 ? (
                ...
                reports.map((report) => (
                  <tr key={report.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <div>Opening: ₹{report.opening_balance}</div>
                      <div>Closing: ₹{report.closing_balance}</div>
                    </td>
                  </tr>
                ))
              )}
